...
export default function Profile() {
    ...
    axios
      ...
      .then((response) => {
        if (response.status === 302 || response.status === 200) {
          setTotalPages(Math.ceil(response.data.users / 10));
        } else {
          setError(true);
        }
        setLoading(false);
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      axios
        .get(`${process.env.API_URL}/users?page=${navPage}&size=10`)
        .then((response) => {
          if (response.status === 302 || response.status === 200) {
                ...
                <Reveal keyframes={customAnimation} duration={225} cascade>
                  {Array.isArray(leaderboard) &&
                    leaderboard.map((user: User, index: number) => (
                      <Link key={index} href={`/profile/${user.info.id}`}>
                        <div
                          className={`LeaderboardEntry ${
                            (index + 1) % 2 === 0 ? undefined : "bg-[#0000003d]"
                          } border-b border-[#0000003d]`}
                          style={{
                            backgroundImage: !user.info.images.banners
                              .horizontal
                              ? ``
                              : `url(/api/${user.info.id}/hor)`,
                          ...
                          <div className="LBEntryText LBU">
                            <div className="relative overflow-visible mr-5">
                              <Image
                                ...
                                loading="eager"
                                ref={
                                  !user.info.images.border?.includes(
                                    "glitch_border.gif",
                                  )
                                    ? undefined
                                    : glitch.ref
                                ...
                                src={
                                  !user.info.images.avatar
                                    ? "/assets/images/PFPPlaceholder.png"
                                    : `${process.env.API_URL}/assets/${user.info.id}/avatar`
                                }
                                alt="Profile Picture"
                                width={32}
                                height={32}
                                ...
                                className="LBPFP"
                              />
                              {user.info.images.border && (
                                <Image
                                  loading="eager"
                                  priority={true}
                                  src={`/assets/images/users/borders/${user.info.images.border}`}
                                  alt="Border Image"
                                  className="LBBorder"
                                  width={220}
                                  height={220}
