    ...
    if (!antiSpam) {
      ...
      await axios
        .post(`${process.env.PUBLIC_URL}/api/crafting/craft`, {
          item1: recipe.input1.id,
          item2: recipe.input2.id,
          token: session.jwt,
        })
        .then((response) => {
          if (response.status === 302 || response.status === 200) {
            const recipesToSubtractFrom = recipes.filter((r) => {
              const hasItem1 =
                r.input1.id === recipe.input1.id ||
                r.input2.id === recipe.input1.id;
              const hasItem2 =
                r.input1.id === recipe.input2.id ||
                r.input2.id === recipe.input2.id;
